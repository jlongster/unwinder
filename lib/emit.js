...
Ep.getMachine = function() {
  return {
    contextId: this.contextId.name,
    ast: this.getDispatchLoop()
  };
};
...
Ep.getDispatchLoop = function() {
  ...
  return b.doWhileStatement(
    ...
    b.binaryExpression('===', this.contextProperty("state"),
                       // optimization: hardcode the value of VM.EXEUCTING here
                       b.literal(3))
  );
};
...
Ep.explodeStatement = function(path, labelId) {
  ...
  case "DebuggerStatement":
    ...
    self.emitAssign(self.contextProperty("state"),
                    b.memberExpression(b.identifier('VM'), b.identifer('SUSPENDED')));
  ...
  case "CallExpression":
    ...
    function calleeName(expr) {
      if(n.Identifier.check(expr)) {
        ...
        return calleeName(expr.object) +
          (expr.computed ? '[' : '.') +
          calleeName(expr.property) +
          (expr.computed ? ']' : '');
      }
      else if(n.FunctionExpression.check(expr) && expr.id) {
        return expr.id.name;
      }
      else {
        return '<func>';
    ...
    self.emitAssign(
      ...
      b.arrayExpression(
        [b.literal(calleeName(newCallee)),
